...
pub struct SshSession {
    handle: Arc<Handle<RivetHandler>>,
    host: String,
    port: u16,
    username: String,
}
...
impl SshSession {
    ...
    pub async fn connect(conn: &Connection) -> Result<Self, SshError> {
        ...
        let mut handle = client::connect(Arc::new(config), &*addr, RivetHandler::new())
            .await
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let mut handle = client::connect_stream(Arc::new(config), stream, RivetHandler::new())
            .await
    ...
    pub async fn disconnect(&self) -> Result<(), SshError> {
        ...
        self.handle
            .disconnect(Disconnect::ByApplication, "rivet disconnect", "en")
            .await
    ...
    pub fn handle(&self) -> &Handle<RivetHandler> {
        &self.handle
    }

    /// Get the handle wrapped in Arc for tunnel operations.
    pub fn handle_arc(&self) -> Arc<Handle<RivetHandler>> {
