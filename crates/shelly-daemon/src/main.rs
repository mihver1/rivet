...
use shelly_core::protocol::{log_dir, pid_file_path, shelly_dir, socket_path};
use tokio::signal;
...
async fn main() {
    ...
    // Ensure ~/.shelly directory exists
    let shelly = shelly_dir();
    if let Err(e) = std::fs::create_dir_all(&shelly) {
        error!(error = %e, "failed to create shelly directory");
        std::process::exit(1);
    }
    ...
    {
        let vault_dir = shelly_core::protocol::vault_dir();
        let store = shelly_vault::store::VaultStore::new(vault_dir);
...
fn setup_tracing() {
    ...
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("info,shelly_daemon=debug,shelly_ssh=debug"));
