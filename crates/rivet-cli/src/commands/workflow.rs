...
use rivet_core::workflow::{Workflow, WorkflowResult};
...
pub async fn list() -> Result<(), CliError> {
    ...
    if workflows.is_empty() {
        println!("No workflows. Import one with: rivet workflow import <file.yaml>");
        return Ok(());
    }
...
pub async fn show(args: &[String]) -> Result<(), CliError> {
    ...
    for (i, step) in wf.steps.iter().enumerate() {
        let action_desc = match &step.action {
            rivet_core::workflow::StepAction::Exec(e) => format!("exec: {}", e.command),
            rivet_core::workflow::StepAction::Upload(t) => {
                format!("upload: {} -> {}", t.local_path, t.remote_path)
            }
            rivet_core::workflow::StepAction::Download(t) => {
                format!("download: {} -> {}", t.remote_path, t.local_path)
            }
        };

        let mut extras = Vec::new();
        if step.on_failure != rivet_core::workflow::OnFailure::Abort {
...
pub async fn run(args: &[String]) -> Result<(), CliError> {
    // Usage: rivet workflow run <name> <connection|--group group-name> [--var key=value ...]
    if args.is_empty() {
