...
pub async fn create(args: &[String]) -> Result<(), CliError> {
    // Usage: rivet tunnel create <connection> -L|-R|-D <spec>
    if args.len() < 3 {
        eprintln!("Usage: rivet tunnel create <connection> -L|-R|-D <spec>");
        eprintln!();
        eprintln!("Examples:");
        eprintln!("  rivet tunnel create prod-db -L 5432:localhost:5432");
        eprintln!("  rivet tunnel create gateway -D 1080");
        eprintln!("  rivet tunnel create web -R 8080:localhost:3000");
