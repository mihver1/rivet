...
use rivet_core::protocol::{JsonRpcError, JsonRpcRequest, JsonRpcResponse, pid_file_path, socket_path};
...
pub async fn ensure_daemon_running() -> Result<(), ClientError> {
    ...
    // Spawn rivetd in the background
    ...
    std::process::Command::new(&rivetd)
        ...
        .spawn()
        .map_err(|e| ClientError::Protocol(format!("failed to start rivetd: {e}")))?;
...
/// Find the rivetd binary.
fn which_rivetd() -> Result<std::path::PathBuf, ClientError> {
...
impl std::fmt::Display for ClientError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ClientError::DaemonNotRunning => {
                write!(f, "daemon is not running (start with: rivet daemon start)")
            }
