...
use shelly_core::protocol::{JsonRpcRequest, JsonRpcResponse, pid_file_path, socket_path};
...
pub async fn ensure_daemon_running() -> Result<(), McpClientError> {
    ...
    std::process::Command::new(&shellyd)
        ...
        .spawn()
        .map_err(|e| McpClientError::Protocol(format!("failed to start shellyd: {e}")))?;
...
fn which_shellyd() -> Result<std::path::PathBuf, McpClientError> {
...
impl std::fmt::Display for McpClientError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            McpClientError::DaemonNotRunning => {
                write!(f, "Shelly daemon is not running. Start it with: shelly daemon start")
            }
