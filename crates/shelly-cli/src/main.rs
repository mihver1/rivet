    ...
    match resolved_cmd {
        ...
        Resolved::NotFound => {
            ...
            if let Some(suggestion) = suggest_command(&tree, first) {
                ...
                eprintln!("Unknown command: '{first}'. Run 'shelly --help' for usage.");
            }
            std::process::exit(2);
        }
    }
...
fn print_help() {
    println!(
        "shelly {} — SSH connection manager",
        env!("CARGO_PKG_VERSION")
    );
    println!();
    println!("USAGE:");
    println!("  shelly <command> [args...]");
    println!("  Commands support Cisco IOS-style prefix matching (e.g., 'co l' = 'conn list')");
    println!();
    println!("COMMANDS:");
    println!("  daemon start|stop|status    Manage the shellyd daemon");
    ...
    println!();
    println!("EXAMPLES:");
    println!("  shelly daemon start          Start the daemon");
    println!("  shelly v u                   Unlock vault (prefix match)");
    println!("  shelly co l                  List connections (prefix match)");
    println!("  shelly ssh prod-web          SSH to 'prod-web'");
    println!("  shelly exec prod-web uptime  Run 'uptime' on prod-web");
    println!("  shelly scp up prod-web ./f.txt /tmp/f.txt");
