...
pub async fn upload(args: &[String]) -> Result<(), CliError> {
    if args.len() < 3 {
        return Err(CliError::MissingArgument(
            "usage: shelly scp upload <connection> <local_path> <remote_path>".into(),
        ));
    }
...
pub async fn download(args: &[String]) -> Result<(), CliError> {
    if args.len() < 3 {
        return Err(CliError::MissingArgument(
            "usage: shelly scp download <connection> <remote_path> <local_path>".into(),
        ));
    }
...
async fn resolve_connection(
    ...
    name: &str,
) -> Result<shelly_core::connection::Connection, CliError> {
