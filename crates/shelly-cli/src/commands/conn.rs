...
pub async fn list() -> Result<(), CliError> {
    ...
    if connections.is_empty() {
        println!("No connections. Add one with: shelly conn add");
        return Ok(());
    }
    ...
    for conn in &connections {
        let auth_type = match &conn.auth {
            shelly_core::connection::AuthMethod::Password(_) => "password",
            shelly_core::connection::AuthMethod::PrivateKey { .. } => "key",
            shelly_core::connection::AuthMethod::KeyFile { .. } => "keyfile",
            shelly_core::connection::AuthMethod::Agent => "agent",
            shelly_core::connection::AuthMethod::Certificate { .. } => "cert",
            shelly_core::connection::AuthMethod::Interactive => "interactive",
        };
...
pub async fn add() -> Result<(), CliError> {
    ...
    let auth = match auth_choice.as_str() {
        "2" => {
            ...
            shelly_core::connection::AuthMethod::Password(password)
        }
        "3" => {
            ...
            shelly_core::connection::AuthMethod::KeyFile {
                path: path.into(),
                passphrase,
            }
        }
        _ => shelly_core::connection::AuthMethod::Agent,
    };
