...
pub fn derive_kek(password: &str, params: &Argon2Params) -> Result<[u8; KEY_LEN]> {
    let argon2 = Argon2::new(
        ...
        .map_err(|e| ShellyError::CryptoError(format!("argon2 params: {e}")))?,
    );

    let mut kek = [0u8; KEY_LEN];
    argon2
        .hash_password_into(password.as_bytes(), &params.salt, &mut kek)
        .map_err(|e| ShellyError::CryptoError(format!("argon2 hash: {e}")))?;
...
pub fn encrypt_aes_gcm(key: &[u8; KEY_LEN], plaintext: &[u8]) -> Result<Vec<u8>> {
    let cipher =
        Aes256Gcm::new_from_slice(key).map_err(|e| ShellyError::CryptoError(e.to_string()))?;
    ...
    let ciphertext = cipher
        ...
        .map_err(|e| ShellyError::CryptoError(format!("encrypt: {e}")))?;
...
pub fn decrypt_aes_gcm(key: &[u8; KEY_LEN], data: &[u8]) -> Result<Vec<u8>> {
    if data.len() < NONCE_LEN + TAG_LEN {
        return Err(ShellyError::CryptoError("data too short".into()));
    }
    ...
    let plaintext = cipher
        ...
        .map_err(|_| ShellyError::InvalidPassword)?;
...
pub fn decrypt_dek(kek: &[u8; KEY_LEN], encrypted: &[u8]) -> Result<[u8; KEY_LEN]> {
    ...
    if plaintext.len() != KEY_LEN {
        plaintext.zeroize();
        return Err(ShellyError::CryptoError("invalid DEK length".into()));
    }
