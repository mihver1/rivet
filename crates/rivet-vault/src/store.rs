...
impl VaultStore {
    ...
    pub fn init(&self, password: &str) -> Result<()> {
        if self.is_initialized() {
            return Err(RivetError::VaultAlreadyInitialized);
        }
        ...
        let config_toml =
            toml::to_string_pretty(&config).map_err(|e| RivetError::SerializationError(e.to_string()))?;
    ...
    pub fn unlock(self, password: &str) -> Result<UnlockedVault> {
        if !self.is_initialized() {
            return Err(RivetError::VaultNotInitialized);
        }

        // Read vault config
        let config_str = fs::read_to_string(self.vault_dir.join("vault.toml"))?;
        let config: VaultConfig =
            toml::from_str(&config_str).map_err(|e| RivetError::SerializationError(e.to_string()))?;
...
impl UnlockedVault {
    ...
    pub fn load_entity<T: DeserializeOwned>(&self, subdir: &str, id: &Uuid) -> Result<T> {
        ...
        if !path.exists() {
            return Err(RivetError::ConnectionNotFound(id.to_string()));
        }
    ...
    pub fn find_connection_by_name(&self, name: &str) -> Result<Connection> {
        ...
        connections
            .into_iter()
            .find(|c| c.name == name)
            .ok_or_else(|| RivetError::ConnectionNotFound(name.into()))
    }
    ...
    pub fn find_group_by_name(&self, name: &str) -> Result<Group> {
        ...
        groups
            .into_iter()
            .find(|g| g.name == name)
            .ok_or_else(|| RivetError::GroupNotFound(name.into()))
    }
    ...
    pub fn change_password(&self, old_password: &str, new_password: &str) -> Result<()> {
        ...
        // Write updated files
        let config_toml =
            toml::to_string_pretty(&new_config).map_err(|e| RivetError::SerializationError(e.to_string()))?;
...
mod tests {
    ...
    fn test_vault_not_initialized() {
        ...
        assert!(matches!(result, Err(RivetError::VaultNotInitialized)));
    }
