/// Rivet MCP Server — exposes SSH management tools to Claude and other LLMs
/// via the Model Context Protocol (stdio transport).
///
/// Usage: rivet-mcp (reads JSON-RPC from stdin, writes to stdout)
mod client;
mod tools;
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("rivet_mcp=debug".parse().unwrap()),
        )
        .with_writer(std::io::stderr)
        .with_target(false)
        .init();

    debug!("rivet-mcp starting");
...
mod tests {
    ...
    fn test_initialize_response() {
        ...
        assert_eq!(result["serverInfo"]["name"], "rivet");
    ...
    fn test_tools_list() {
        ...
        assert!(names.contains(&"show_credential"));
    }
